...
function showArticle(url) {
    ...
    if (url.substr(-1) != '/' && url.indexOf('archives') != -1 && url.substr(-5) != '.html') {
        url += "/"
    }
...
function MoveCheck(inEl, outEl) {
    ...
    if (inEl == "#archive") {
        setTimeout(function () {
            document.querySelector(".archive-container").style.overflowX = "scroll";
        }, 800);
    }
}
        ...
        if (backArchive == true) {
            ...
            setTimeout(function () {
                if (document.querySelector('.category-container>*')) {
                    document.querySelectorAll('.category-container>*').forEach((e) => {
                        e.remove()
                    })
                }
                ;document.querySelector(".archive-container").style.overflowX = "scroll";
...
function sendComment() {
    ...
    fetch(form.action, {credentials: 'include', method: "POST", body: data}).then(function (data) {
        return data.text()
    }).then(function (text) {
        nowSending = false;
        if (text.indexOf('Error') != -1) {
            ...
            if (newComment.querySelector('.comment-children')) {
                newComment.removeChild(newComment.querySelector('.comment-children'))
            }
            ;
